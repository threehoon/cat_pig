...
export function collectNicknames(post: Post, comments: Comment[]): string[] {
  ...
  const add = (name: string | null) => {
    ...
    if (nickname && names.indexOf(nickname) === -1) names.push(nickname)
  }
...
export function collectMentions(post: Post, comments: Comment[], currentUserId: string | null): MentionUser[] {
  ...
  const add = (id: string, nickname: string | null, avatar: string | null) => {
    if (!id || seen[id] || id === currentUserId) return
    seen[id] = true
    list.push({ id, nickname: nickname || '用户', avatar: avatar || brandAssets.avatarDefault })
  }
...
export function groupComments(
  ...
  items.forEach((item, index) => {
    ...
    if (item.parent_id && byId[item.parent_id]) byId[item.parent_id].replies.push(view)
    else roots.push(view)
  })
  return roots
}

export function patchPlaying(list: CommentView[], id: string): CommentView[] {
  return list.map((item) => ({ ...item, playing: item.id === id, replies: patchPlaying(item.replies, id) }))
}
...
export function patchCommentLike(list: CommentView[], id: string, liked: boolean, likeCount: number): CommentView[] {
  return list.map((item) =>
    item.id === id
      ...
      : item.replies.length
        ? { ...item, replies: patchCommentLike(item.replies, id, liked, likeCount) }
        : item,
  )
}

export function mentionNames(users: MentionUser[]): string[] {
  return users.map((item) => item.nickname)
}
